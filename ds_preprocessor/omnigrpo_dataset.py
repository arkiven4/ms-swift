...
def crop_and_convertJPG(path_file, max_size=2048):
    ...
    def _generate_examples(self, csv_files, data_dir):
        ...
        for csv_file in csv_files:
            ...
            for _, row in df.iterrows():
                ...
                # audio_name = row.get("audio_file_name")
                # if pd.notna(audio_name):
                #     audio_path = os.path.join(data_dir, audio_name)
                #     if os.path.exists(audio_path):
                #         example["audios"] = audio_path
